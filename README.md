...
2.  **Фильтрация:**
3.    * *Classic:* Regex-фильтр на уголовные дела.
      * *Neural:* Прогон через Qwen2.5 с жестким системным промптом для выявления роли банка.
4.  **Индексация:** Рекурсивный чанкинг (`chunk_size=1000`, `overlap=300`) и создание векторного хранилища.
5.  **RAG-пайплайн:** 1) Юзер задает вопрос 2) Гибридный поиск 3) Реранкинг 4) Генерация ответа через локальную LLM.
6.  **Оценка:** Замер метрик через `Ragas`.
