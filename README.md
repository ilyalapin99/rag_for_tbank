# RAG: Фильтрация и QA-поиск по судебной практике (SudAct)
...
### Сложность:
Проблема "Шумовых упоминаний": При анализе судебных дел по запросу крупной организации (например, банка или телеком-оператора), стандартный поиск выдает тысячи документов, где компания упоминается лишь как «третье лицо» или «оператор перевода». Для юриста это информационный мусор.
...
* `ragas_final_results.csv` — итоговые метрики.


## Как запустить
1. Собрать данные:
   * `sudact_parser.py` (требуется установка Chrome).
